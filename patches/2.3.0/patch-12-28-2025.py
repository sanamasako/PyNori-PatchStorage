def EnterBattle(Challenge=None):
	global Route, LocCode, LocCodes, location, npc_battle, CurE, EnemD, spares, EnemyHP, EnemyMaxHP, NeedsInteract, Satisfied, EnemyLvl, EnemyATK, EnemyDEF, IsbattleNPC, satisfaction, ActiveQuirk, Quirk1, Quirk2, DualQMode, EXP, EnemyPop, AlreadyDecrementedSleep, AlreadyDecrementedCaffeine, PStoggle
		...
		try:
			...
			if Challenge not in ["ow", "battle"]:
				while StartBattle is False:
					...
					try: CurE_PopCheck.read(GetEnemyPopINIPath())
			...
			if not Challenge and EnemStat["Powerscaling"]["enabled"] == "True" and EnemStat["Powerscaling"]["mode"] not in [None, "null"] and LvlDiff//int(EnemStat["Powerscaling"]["ratio"].split(":")[-1]) > 0 and PStoggle:
				...
				if EnemPowAmped or EnemPowNerfed: print(f"You're at a{" dis" if EnemyLvl > Lvl else "n "}advantage, so your opponent has become {"weak" if EnemyLvl > Lvl else "strong"}er!"); pause()
