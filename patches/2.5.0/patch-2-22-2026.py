def EnterBattle(Challenge=None):
	global Route, LocCode, LocCodes, location, npc_battle, CurE, EnemD, spares, EnemyHP, EnemyMaxHP, NeedsInteract, Satisfied, EnemyLvl, EnemyATK, EnemyDEF, IsbattleNPC, satisfaction, ActiveQuirk, Quirk1, Quirk2, DualQMode, EXP, EnemyPop, AlreadyDecrementedSleep, AlreadyDecrementedCaffeine, PStoggle, EnemyPopINI, SpookyTime
	...
	try:
		if SpookyTime: global PlayingDead; PlayingDead = False
	except: pass
	...
	if ActiveQuirk == "Enemies Fear You" or (DualQMode and (Quirk1 == "Enemies Fear You" or Quirk2 == "Enemies Fear You")) and not Challenge: Log("Whoops! You're very scary so no battles for you."); print("Huh? The enemy ran away..."); spares += 1; pause(); overworld = None; BattleOrNot = 16
		...
		try:
			...
			try:
				if SpookyTime: pass
			except: SpookyTime = false
			if Challenge not in ["ow", "battle"]:
				while not StartBattle:
					...
					if not SpookyTime:
						...
						if EnemyPop[CurE] == 0: Extinct.append(CurE); continue
						StartBattle = True
			...
			if not Challenge and EnemStat["Powerscaling"]["enabled"] == "True" and EnemStat["Powerscaling"]["mode"] not in [None, "null"] and LvlDiff//int(EnemStat["Powerscaling"]["ratio"].split(":")[-1]) > 0 and PStoggle:
				...
				ScaleCount = LvlDiff//MultPerLevel; ScaleCount = 0 if not TestPowerscale(EnemyMaxHP, EnemyLvl, EnemyATK, EnemyDEF, mode=EnemStat["Powerscaling"]["mode"], count=ScaleCount, psType=EnemStat["Powerscaling"]["method"], mult=PowerscaleMultiplier) else ScaleCount
				Log(f"The powerscale will be performed {ScaleCount} time{"s" if ScaleCount != 1 else ""}.\nEnemy stats: {EnemyMaxHP} HP, {EnemyATK} ATK {EnemyDEF} DEF")
			...
			pronoun1 = EnemStat.get('pronoun1', 'It'); pronoun2 = EnemStat.get('pronoun2', 'Its'); pronounPossessive = EnemStat.get('pronounPossessive', 'Its'); Log("Loaded enemy pronouns (yes, that's an official thing LOL).")
			...
			SoundCh.SFX.play(tef); Log(f"AN ERROR OCCURRED WHILE LOADING THE BATTLE: {e}")
