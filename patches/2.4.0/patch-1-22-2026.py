def OptionsMenu():
	global config, AdvText, Diff, debug, cheating, cheatymccheatcheat, BatStat, sisas, OGsfx, MGtoggle, SplashText, PStoggle, MultipleSaves, ShowHPBars, StairsNerf, PinorINI, RunsINI, InventorINI, EnemyPopINI
	try:
	...
	while True:
		...
		try:
			...
			if OptInp.endswith(".desc"): OptionDesc(Option=OptInp.split(".")[0]); pause(); continue
			if OptInp in ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"]: OptInp = int(OptInp)
			if OptInp in ["a", "b", "c", "d", "e", "f"]: OptInp = OptInp.upper()
			if OptInp not in [1, 2, 3, 4, 5, 6, 7, 8, 9, 0, "A", "a", "B", "b", "C", "c", "D", "d", "E", "e", "F", "f"]: raise ValueError
		except ValueError: print("Please choose either A/B/C/D/E or a single digit number."); pause(); continue
		options = {"A": WeatherOptions, "B": SplashTextToggle, "C": PowerscaleToggle, "D": MultiSaveToggle, "E": HealthBarsToggle, "F": StairsNerfToggle, 1: DiffChanger, 2: AdvTextToggle, 3: BatStatToggle, 4: CheatChance if cheating == "NO" else lambda: print("You are already cheating."), 5: lambda: not debug, 6: KillSystemSwitch, 7: SISAS, 8: ogSFX, 9: ToggleMG, 0: TitleScreen}
