def BuildWeather(Silent=True, ReturnCurrent=False, Bypass=False):
	def CompareHours():
		try:
		...
		except: global Weather; hour = Date.hour; Weather = None; return False 
	global WeatherEnabled, Weather
	try:
		try: Log(f"Weather was built recently? {CompareHours() and isinstance(Weather, dict)}\nLast check was an hour ago?: {CompareHours()}\nWeather compiled?: {isinstance(Weather, dict)}")
        except Exception as e: Weather = None; Log(f"Failed to check if weather was built recently. The weather data has been wiped.\nException: {e}")
		if CompareHours() and isinstance(Weather, dict): raise FileNotFoundError
		if os.path.exists(GetCustomFilePath("weatherconfig.ini")): GetHemi = configparser.ConfigParser(); GetHemi.read(GetCustomFilePath("weatherconfig.ini")); WeatherConf = GetHemi["Customisation"]; SavedHemi = WeatherConf["hemisphere"]; Log(f"Hemisphere setting: {SavedHemi}"); WeatherEnabled = GetHemi.getboolean("Customisation", "weather events")
		else: SavedHemi = "Auto"
		if not WeatherEnabled and not Bypass: Log("Attempted to build weather while Weather Events were disabled. Returning None."); return None
		...
		except Exception as e: Log(f"EXCEPTION WHEN FETCHING WEATHER: {e}")
		...
		Weather = {
			...
			"Conditions": flags}; Log("Weather built successfully."); return Weather
	except FileNotFoundError: Log("Weather was built within the last hour. Skipping weather rebuilding."); return Weather
